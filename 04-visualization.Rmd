...
# fn is the function to summarize at each step

# w is a column to use as weights

paper_hist <- function(df, vars, fn, w=NULL){
    n_ungroup <- length(vars)
    
    if (!is.null(w)){
        stat1 <- df %>% 
        group_by(across(vars)) %>%
        summarise(paper_level = fn(repro_score))
    if (length(vars)>1){
        for (i in c(0:(length(vars)-2))){
        stat1 <- stat1 %>%
            ungroup(vars[length(vars)-i]) %>%
            summarise(paper_level=fn(paper_level))
    }
    }
    } else{
        stat1 <- df %>% 
        group_by(across(vars)) %>%
        summarise(paper_level = fn(repro_score,w=w))
    if (length(vars)>1){
        for (i in c(0:(length(vars)-2))){
        stat1 <- stat1 %>%
            ungroup(vars[length(vars)-i]) %>%
            summarise(paper_level=fn(paper_level,w=w))
    }
    }
    }
...
```{r}
paper_hist(dis_df, c("similar_title","reproduction_id"),mean)+
...
### Levels of Reproducibility (Abandoned Included)
```{r}
...
paper_hist(dis_aband,c("similar_title","reproduction_id"),mean)+
    labs(title = "Distribution of Reproduction Scores: Paper Level",
             x = "Reproducibility Score",
             y = "Count")
```

### Levels of Reproducibility, Max within paper
```{r}
paper_hist(dis_df,c("similar_title"),max)+
    labs(title = "Distribution of Max Reproduction Scores: Paper Level",
             x = "Reproducibility Score",
             y = "Count")
