...
abandoned_scores <- abandoned_df %>%
    mutate(paper_level=1) %>%
    group_by(similar_title) %>%
    summarize(paper_level=mean(paper_level))

stat2 <- rbind.data.frame(stat1,abandoned_scores)
...
stat2 %>% ggplot(aes(x = paper_level)) +
  geom_histogram(binwidth = 1,
                 color = "black",
                 fill = "gray") +
  labs(title = "Distribution of Reproduction Scores: Paper Level",
       x = "Reproducibility Score",
       y = "Count") +
  coord_cartesian(ylim =  c(0, 1.1*level_max_abd)) +
  scale_x_continuous(breaks = 1:10) +
    theme(panel.background = element_rect(fill="white"),
          panel.grid.major.y = element_line(color="grey80"),
          panel.grid.minor.y = element_line(color="grey80"),
