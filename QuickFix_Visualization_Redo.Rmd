...
```{r}
main_hist <- paper_hist(tidy_di_df, c("doi", "reproduction_id"), list(mean, mean)) +
...
```{r}
abandoned_scores <- abandoned_repro %>%
                    select(doi,reproduction_id) %>%
                    mutate(repro_score=1)

dis_aband <- rbind.data.frame(
                    select(tidy_di_df,doi,reproduction_id,repro_score),
                    abandoned_scores)
...
```{r}
max_hist <- paper_hist(tidy_di_df, c("doi"), list(max)) +
...
```{r}
min_hist <- paper_hist(tidy_di_df, c("doi"), c(min))+
...
country_counts <- tidy_di_df %>% 
  group_by(reproduction_id) %>% 
  slice(1) %>%
  left_join(reproducer_char, by = c("reproducer_id", "reproduction_id")) %>%
